...
Perform an in-depth Previous Year Questions (PYQs) analysis on the uploaded documents.

Return ONLY valid JSON:
{
  "overview": "Summary of PYQ trends and exam pattern",
  "unitWeightage": [
    { "unit": "Unit 1: Fundamentals", "percentage": 30, "topicCount": 5 }
  ],
  "frequentlyAskedTopics": [
    { "topic": "Core Topic", "occurrenceCount": 6, "importance": "High" }
  ],
  "repeatedQuestions": [
    "Repeated Question 1",
    "Repeated Question 2"
  ...
  "predictedQuestions": [
    "Predicted Question for upcoming exam 1",
    "Predicted Question for upcoming exam 2"
  ]
}
...
${context}`,
  ...
  formulaSheet: (context) => `
${RAG_SYSTEM_INSTRUCTION}

Extract all key formulas, mathematical equations, constants, and quick reference identities from the document.
...
Return ONLY valid JSON:
{
  ...
  "formulas": [
    {
      "name": "Formula Name",
      "expression": "e.g., E = mc^2 or F = m * a",
...
Generate an ultimate 1-Page Last Minute Exam Cheat Sheet summarizing key concepts, definitions, syntax/formulas, and common pitfalls.

Format as structured Markdown with tables and quick memory boxes.

Document Context:
${context}`,

  codingQuestions: (context) => `
${RAG_SYSTEM_INSTRUCTION}

Generate coding & algorithm practice problems based on algorithms, data structures, or code logic discussed in the study context.
  ...
  "codingQuestions": [
    {
      ...
      "title": "Problem Title",
      "difficulty": "Medium",
      "problemStatement": "Detailed description of the programming problem",
      "inputOutputFormat": "Input: ... Output: ...",
      "sampleCase": { "input": "...", "output": "..." },
      "cppSolution": "C++ code string...",
      "pythonSolution": "Python code string...",
      "javaSolution": "Java code string...",
      "timeComplexity": "O(N)",
      "spaceComplexity": "O(1)"
    }
  ]
...
${context}`,
  ...
  personalizedStudyPlan: (context, { examDate = '14 days', weakTopics = [] }) => `
${RAG_SYSTEM_INSTRUCTION}

Create a personalized day-by-day Study Schedule leading up to exam date in ${examDate}. 
Weak areas highlighted: ${weakTopics.join(', ') || 'General revision'}.

Return ONLY valid JSON:
{
  "planTitle": "Personalized Study Roadmap (${examDate})",
  "strategy": "High yield topic prioritization strategy",
  "dailySchedule": [
    {
      "day": 1,
      "focusTopic": "Topic Name",
      "tasks": ["Read Notes", "Solve 10 MCQs", "Revise Flashcards"],
      "estimatedHours": 2.5
    }
  ],
...
${context}`,
  ...
  revisionPlanner: (context) => `
${RAG_SYSTEM_INSTRUCTION}

Generate a Spaced Repetition Revision Schedule based on the syllabus context.

Return ONLY valid JSON:
{
  "scheduleName": "Spaced Repetition Revision Plan",
  "phases": [
    {
      "phase": "Day 1 (Immediate Review)",
      "topics": ["Core Definitions", "Primary Formulas"],
      "action": "Solve 5 Flashcards & 5 MCQs"
    },
    {
      "phase": "Day 3 (Second Review)",
      "topics": ["Detailed Concepts"],
      "action": "Take 10 Min Quiz"
    },
    {
      "phase": "Day 7 (Weekly Mastery)",
      "topics": ["PYQs & Hard Problems"],
      "action": "Full Practice Test"
    }
  ]
}
